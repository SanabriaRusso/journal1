...
%Further, by applying simple extensions to the backoff mechanism of CSMA/ECA it is able to support many contenders in a collision-free schedule, surpassing the achieved throughput of CSMA/CA and providing short-term throughput fairness among contenders.
...
%This work describes CSMA/ECA and its extensions that allow it to achieve a collision-free schedule with many contenders in non-saturated conditions, as well as its performance when coexisting with CSMA/CA nodes. Further, the effects of imperfect clocks over CSMA/ECA's deterministic backoff mechanism and its consequences when attempting to implement the protocol in real hardware are also analysed. 
...
%handle many contenders without sacrificing short-term throughput fairness. 
...
The \emph{JFI for CSMA/ECA} and \emph{JFI for CSMA/CA} curves in Figure~\ref{fig:BECA} show the Jain's Fairness index for both protocols. Both protocols have a JFI equal to one, which suggests that the available throughput is shared evenly among all stations.
	...
	%greater throughput than CSMA/CA. Figure~\ref{fig:ECA-vs-DCF-maxAgg} shows the aggregated throughput for CSMA/ECA$_{\text{Hys+FS}}$ and CSMA/CA with nodes sending $2^m$ packets in each attempt. This behavior will be called CSMA/CA$_{\text{MaxAg}}$ and CSMA/ECA$_{\text{Hys+FS}}$ (Max Ag.) for CSMA/CA and CSMA/ECA$_{\text{Hys+FS}}$, respectively from here forth.
	...
	where $s_{k}(l_{k})$ and $s_{k-1}(l_{k-1})$ are the throughput achieved by the nodes at the $k$-th and $(k-1)$-th backoff stages sending $l_{k} = 2^{k}$ and $l_{k-1} = 2^{k-1}$ packets respectively. These are given by:
	...
	The \emph{Lower-bound} is derived from considering the operation of an ideal CSMA/ECA$_{\text{Hys+FS}}$ network. Nodes use the minimum backoff stage possible and aggregate proportionally, thus yielding the minimum throughput achievable by a CSMA/ECA$_{\text{Hys+FS}}$ network. It is computed following~(\ref{eq:bound}).
	
	The \emph{Upper-bound} is obtained from considering the operation of a network using CSMA/ECA$_{\text{Hys+FS}}$, but forcing nodes to use the maximum backoff stage for determining the cycle length and consequently the level of aggregation. It is also computed using~(\ref{eq:bound}) but considering $k=m$ for all nodes.
	
	The maximum throughput achievable is the result of deactivating the Fair Share rules by forcing nodes to use maximum aggregation regardless of their backoff stage. This is called \emph{Maximum Aggregation (Hys+MaxAg)} in Figure~\ref{fig:ECA-bounds-comparison}. It can be derived from~(\ref{eq:bound}) considering $l_{m} = 2^{m}$.
	
	%The \emph{Upper-bound} is obtained from considering the operation of a network using CSMA/ECA$_{\text{Hys+MaxAg}}$. In opposition to the Lower-bound, here the nodes are assumed to aggregate as much as possible in spite of using the minimum schedule, hence yielding the maximum throughput the system can achieve. The Upper-bound is also computed using~(\ref{eq:bound}) but considering $l_{\alpha} = 2^{m}$, regardless of the backoff stage.
%		
...
\section{Results}\label{results}
	...
	%the arrival of a packet to the MAC queue up until an acknowledgment is received for such packet(s).
...
\begin{itemize}
	\item The performance of a CSMA/ECA$_{\text{Hys+FS}}$ network in presence of other neighbouring wireless networks and hidden nodes: transmissions from other network may negatively affect the maintenance of the collision-free schedule when not all devices in the CSMA/ECA$_{\text{Hys+FS}}$ WLAN are able to listen to them. As a consequence, not all stations will pause the backoff accordingly, resulting in large slot drifts that in consequence would disrupt the collision-free schedule, approximating CSMA/ECA$_{\text{Hys+FS}}$ performance to CSMA/CA's. A similar effect is expected when in the presence of hidden nodes.
	%this problem is similar to that of hidden nodes. If the channel is found busy the nodes will freeze their respective backoff counters, while those unable to detect the channel busy with the overlapping AP transmission (or hidden node's) will not. This kind of behaviour results in slot drifts that in consequence would disrupt the collision-free schedule and approximate CSMA/ECA$_{\text{Hys+FS}}$ performance to CSMA/CA's.
	...
	\end{itemize}
	%Nevertheless, the AIFS mechanism can still be used. If a fixed AIFS is assigned for CSMA/ECA$_{\text{Hys+FS}}$ stations, the multiple queues and TXOP mechanisms will take care of prioritisation. Furthermore, using the shortest AIFS value would promote CSMA/ECA$_{\text{Hys+FS}}$ adoption by prioritising its traffic over EDCA's.
...
%\begin{itemize}
%	\item Multiple-User Multiple-Input Multiple-Output (MU-MIMO): nodes, like the AP for instance, would be able to send packets to different users using the same transmission slot; further increasing the aggregated throughput of the network.
%	\item Channel bonding: increasing the channel bandwidth also provides a throughput improvement, even though no modification to CSMA/ECA$_{\text{Hys+FS}}$ is required.
%\end{itemize}
%
%Apart from the mentioned above, there are other features that are not yet straightforward supported by CSMA/ECA$_{\text{Hys+FS}}$:
%
%\begin{itemize}
%	\item Traffic differentiation: although priorities using different contention windows in CSMA/ECA$_{\text{Hys+FS}}$ proved to outperform CSMA/CA, other Enhanced Distributed Channel Access (EDCA) mechanisms like the Arbitration Inter-Frame Spacing (AIFS), would not work~\cite{jaumeTD}.
%	\begin{itemize}
%		\item Transmission Opportunity (TXOP): stations with an increased TXOP are able to transmit more packets. APs in CSMA/ECA$_{\text{Hys+FS}}$ can make use of the TXOP feature in order to transmit more than users.
%		\item Multiple Queues:~\cite{jaumeTD} provides performance metrics for two traffic categories. In order to transition to CSMA/ECA$_{\text{Hys+FS}}$ a total of four access categories (AC) should be implemented, as in EDCA.
%	\end{itemize}
%\end{itemize}
%
%Other WLANs features, like security, are MAC agnostic, so its implementation using CSMA/ECA$_{\text{Hys+FS}}$ is straightforward.
%
%The aforementioned features would level CSMA/ECA$_{\text{Hys+FS}}$ features with the CSMA/CA implementation for the current WLAN standard, providing sufficient insight to be considered a MAC replacement in WLANs.
